    ...
    useEffect(() => {
        const fetchSettings = async () => {
            try {
                ...
                // Fallback hardcoded values matching request if fetch fails
                setPlans(prev => prev.map(p => {
                    if (p.id === 'monthly' && p.price === '...') return {
                        ...p,
                        price: '499',
                        features: [
                            'All Free features',
                            'Unlimited session bookings', // Fallback
                            'Direct messaging with mentors',
                            'Priority support',
                            'Access to study materials'
                        ]
                    };
                    if (p.id === 'yearly' && p.price === '...') return {
                        ...p,
                        price: '9999',
                        features: [
                            'All Pro Monthly features',
