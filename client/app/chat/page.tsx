...
function ChatContent() {
    ...
    const scrollToBottom = () => {
        if (messagesEndRef.current) {
            messagesEndRef.current.scrollIntoView({
                behavior: "smooth",
                block: "nearest" // This prevents the whole page from jumping down to the footer
            });
        }
    };
    ...
    useEffect(() => {
        if (withUserId) {
            ...
            markRead(withUserId);
            // Join specific room for the two users
    ...
    const markRead = async (userId: string) => {
        try {
            ...
            fetchPartners(); // Refresh unread badges in the inbox list
        } catch (e) { }
    }
    ...
    useEffect(() => {
        socket.on('receive_message', (data) => {
            ...
            // Always refresh partners to show latest activity in inbox
            fetchPartners();
        });
        ...
        try {
            ...
            if (withUserId) {
                ...
                if (active) setSelectedPartner(active);
                    ...
                    // Brand new chat - fetch partner info directly
                    try {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setNewMessage('');
            fetchPartners(); // Refresh inbox preview snippet
        } catch (error: any) {
    ...
    return (
        <div className="container mx-auto px-4 py-4 max-w-6xl h-[calc(100vh-80px)] overflow-hidden flex flex-col">
            <div className="flex flex-col md:flex-row gap-6 h-full min-h-0">
                            ...
                            partners.map((partner) => (
                                <div
                                    ...
                                    onClick={() => {
                                        ...
                                        router.push(`/chat?with=${partner._id}`);
                                    }}
                ...
                {/* Message Box */}
                <Card className="flex-1 flex flex-col shadow-xl border-brand-100/50 overflow-hidden h-[60vh] md:h-full bg-white">
                    {selectedPartner ? (
                        <>
                            <CardHeader className="bg-white border-b py-4 px-6 z-10 shadow-sm flex-shrink-0">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-full bg-brand-600 text-white flex items-center justify-center font-bold shadow-brand-500/20 shadow-lg">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="text-xs text-slate-500 font-medium capitalize">{selectedPartner.role}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <CardContent className="grow overflow-y-auto p-4 md:p-6 space-y-6 bg-slate-50/30 flex flex-col">
                                ...
                                {messages.map((msg, i) => (
                                    ...
                                    >
                                        <div
                                            className={`max-w-[90%] md:max-w-[75%] px-5 py-3 rounded-2xl text-[15px] leading-relaxed shadow-sm relative ${msg.sender._id === me?._id
                        ...
                        <div className="flex flex-col items-center justify-center grow text-center p-12 bg-slate-50/20">
                            <div className="w-24 h-24 bg-brand-50 rounded-full flex items-center justify-center mb-6 text-brand-300 animate-pulse">
                            ...
                            <p className="text-slate-500 max-w-xs mx-auto text-sm leading-relaxed font-medium">
                                Choose one of your existing chats from the left sidebar or start a new one from a mentor's profile.
                            </p>
                        </div>
                    )}
                </Card>
...
export default function ChatPage() {
    return (
        <Suspense fallback={<div className="flex justify-center items-center h-screen"><Loader2 className="animate-spin text-brand-600" size={48} /></div>}>
            <ChatContent />
        </Suspense>
    );
}
